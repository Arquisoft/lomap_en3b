...
LoMap will provide the users with three privacy levels:
...
The *location* of the data will change *depending on it's privacy level.*:

    * All data the user decides to share with other users apart from his/her self will be stored in LoMaps database.

        ** Friend-Only entries will be flagged so they are not shown if the user is not a contact of the creator.

    * All contributions set as private will be stored in the POD associated to the id of the user.

*The references to Friend  users are stored in the Pod, not in LoMap's database.* That means that, to display friend content, a petition to the pod will be made to ask for the set of friends of a user.
    -> LoMap will retrieve the locations set as friend-only  by each contact from the database and then add them to the user's personal rendered map.

[plantuml,"FriendsContent",png]
----
@startuml
title  Public & Friends-Only content
...
group user adds public location
    User-> LoMap: Adds Location as public
    activate LoMap #LightBlue
    LoMap-> LoMap : gets user id stored in session
    LoMap ->db: inserts location
    deactivate LoMap
end
group user adds friend-only location
    ...
    activate LoMap #LightBlue
    LoMap-> LoMap : gets user id stored in session
    LoMap ->db: inserts location as friend only
    deactivate LoMap
end
note over InruptApi
       When a user witnesses the map , it will show:
            -Posts other users made PUBLIC
...
maps--> LoMap : returns map
deactivate maps
LoMap-> LoMap : adds DB (public user) info to map render
...
LoMap->db:ask for each friend's shared locations
activate db #green
db-->LoMap: returns locations
deactivate db
